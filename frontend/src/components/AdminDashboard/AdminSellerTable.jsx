...
const SellerTable = () => {
  ...
  const fetchSellers = async () => {
    ...
    if (data.length > 0) {
      ...
      setIsPrevDisabled(true); // Disable "Prev" initially
      setIsNextDisabled(data.length < 10); // Disable "Next" if fewer than 10 sellers
    }
  };
  ...
  const handleFetchMoreSellers = async () => {
    ...
    if (newSellers.length > 0) {
      ...
      setIsNextDisabled(newSellers.length < 10); // Disable "Next" if no more sellers
    }
  };
  ...
  const handleFetchPrevSellers = async () => {
    ...
    if (prevSellers.length > 0) {
      setSellers(prevSellers);
      setFirstSellerId(prevSellers[0].id);
      setLastSellerId(prevSellers[prevSellers.length - 1].id);
      setIsNextDisabled(false);
      setIsPrevDisabled(prevSellers.length < 10); // Disable "Prev" if no previous sellers
    }
  };
  ...
  const handleSearch = async () => {
    ...
    if (seller.length > 0) {
      ...
      setIsPrevDisabled(true); // Disable "Prev" if searching
      setIsNextDisabled(true); // Disable "Next" if searching
    } else {
